...
def test_existing_contract_exists_checker_function(dynamodb, lambda_context):
    stepfunctions_event = load_event('lambda/contract_status_checker')
    ...
    ret = contract_exists_checker_function.lambda_handler(stepfunctions_event, lambda_context)
...
@mock.patch.dict(os.environ, return_env_vars_dict(), clear=True)
def test_missing_contract_exists_checker_function(dynamodb, lambda_context):
    stepfunctions_event = load_event('lambda/contract_status_checker')
    ...
    with pytest.raises(ContractStatusNotFoundException) as errinfo:
        contract_exists_checker_function.lambda_handler(stepfunctions_event, lambda_context)
