...
import re
import six
import schema.unicorn_properties_web.publicationapprovalrequested
    ...
    def unmarshall(cls, data, typeName):
        ...
        if type(typeName) == str:
            ...
            if typeName in cls.NATIVE_TYPES_MAPPING:
                ...
                typeName = getattr(schema.unicorn_properties_web.publicationapprovalrequested, typeName)
    ...
    def __unmarshall_model(cls, data, typeName):
        ...
        if typeName._types is not None:
            for attr, attr_type in six.iteritems(typeName._types):
                if data is not None and typeName._attribute_map[attr] in data and isinstance(data, (list, dict)):
                    value = data[typeName._attribute_map[attr]]
                    kwargs[attr] = cls.unmarshall(value, attr_type)

        instance = typeName(**kwargs)

        if isinstance(instance, dict) and typeName._types is not None and isinstance(data, dict):
